...
class Langevin3D():
    ...
    def compute_force(self, r_rel, speed, mass):
        fm = self.force_morse(r_rel) 
        assert fm.shape == (self.dimension,), f"force_morse should return a vector of dimension {self.dimension}"
    ...
    def run(self, n_steps, r_init_relative, v_init=None, T_init=None, mode='langevin'):
        ...
        for _ in range(2, len(time)):
            r_H_new, v_H_new, force_H_new = alg(pos=r_H, speed=v_H, mass=self.m_H, r_rel=np.array(r_rel))
            r_Cl_new, v_Cl_new, force_Cl_new = alg(pos=r_Cl, speed=v_Cl, mass=self.m_Cl, r_rel=-np.array(r_rel))
        ...
        data['potential_energy'] = self.potential_morse(np.sqrt(np.sum(data['r_rel']**2, axis=1)))
